#!/usr/bin/env python3
"""Generate index.html listing all Esketamine daily reports."""
...
html_files = sorted(glob.glob("docs/esketamine-*.html"), reverse=True)
links = ""
for f in html_files[:30]:
    name = os.path.basename(f)
    date = name.replace("esketamine-", "").replace(".html", "")
    try:
...
<title>Esketamine Brain · Esketamine 文獻日報</title>
<style>
  ...
  *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .clinic-link { display: flex; align-items: center; gap: 14px; padding: 14px 20px; background: var(--surface); border: 1px solid var(--line); border-radius: 12px; text-decoration: none; color: var(--text); transition: all 0.2s; }
  .clinic-link:hover { border-color: var(--accent); transform: translateX(4px); }
  .clinic-icon { font-size: 22px; }
  .clinic-name { font-size: 14px; flex: 1; }
  .clinic-arrow { font-size: 16px; color: var(--accent); }
...
<div class="container">
  <div class="logo">🧠</div>
  <h1>Esketamine Brain</h1>
  <p class="subtitle">Esketamine / TRD 文獻日報 · 每日自動更新</p>
  ...
  <footer>
    <p>Powered by PubMed + Zhipu AI · <a href="https://github.com/u8901006/esketamine-brain">GitHub</a></p>
  </footer>
</div>
</body>
</html>""")

index = INDEX_TEMPLATE.substitute(total=total, links=links)
